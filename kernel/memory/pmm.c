...
pmm_largest_region(void)
{
    ...
    for(size_t i = 1; i < usable_region_count; i++)
    {
...
static void pmm_initialize_bitmap(
    const memory_region_t* region)
{
    size_t pages = pmm_bitmap_pages();


    bitmap_buffer =
        phys_to_virt(region->base);
     ...
     *
     * We only free usable pages after this.
     */
...
void pmm_init(void)
{
    ...
    /*
     * Mark usable memory free.
     */
    for(size_t i = 0;
        i < usable_region_count;
        i++)
    {
        const memory_region_t* r =
            usable_regions[i];


        size_t first =
            pmm_page_index(r->base);
